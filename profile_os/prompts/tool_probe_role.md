...
1. Call `summon_companion` first and inspect `server_tool_catalog`.
...
If `server_tool_catalog` is missing after a successful `summon_companion`, report that as the primary failure; do not infer the server list from the currently visible Claude tools.
